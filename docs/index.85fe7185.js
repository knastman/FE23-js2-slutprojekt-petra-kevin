 ...
 */function(e,t){if(e.uid)throw Error('The "uid" field is no longer supported by mockUserToken. Please use "sub" instead for Firebase Auth User ID.');let n=t||"demo-project",i=e.iat||0,r=e.sub||e.user_id;if(!r)throw Error("mockUserToken must contain 'sub' or 'user_id' field!");let s=Object.assign({iss:`https://securetoken.google.com/${n}`,aud:n,iat:i,exp:i+3600,auth_time:i,sub:r,user_id:r,firebase:{sign_in_provider:"custom",identities:{}}},e);return[Q(JSON.stringify({alg:"none",type:"JWT"})),Q(JSON.stringify(s)),""].join(".")}(i.mockUserToken,e.app.options.projectId))),r=s,o.repoInfo_=new tJ(`${t}:${n}`,!1,o.repoInfo_.namespace,o.repoInfo_.webSocketOnly,o.repoInfo_.nodeAdmin,o.repoInfo_.persistenceKey,o.repoInfo_.includeNamespaceInQueryParams,!0),r&&(o.authTokenProvider_=r)}(n,...e)}return n}(e9({apiKey:"AIzaSyDpPuITTDAXymL2UCcisK-nGdAAU0",authDomain:"fe23-slutprojekt-userdb.firebaseapp.com",projectId:"fe23-slutprojekt-userdb",databaseURL:"https://fe23-slutprojekt-userdb-default-rtdb.europe-west1.firebasedatabase.app",storageBucket:"fe23-slutprojekt-userdb.appspot.com",messagingSenderId:"1056096098177",appId:"1:1056096098177:web:716d9daa8907b6bb1d7a80",measurementId:"G-SJNLBSDKQ7"})),sl="usersv2";async function sh(){let e=r8(sa,sl);try{let t=await si(e);if(!t.exists())return[];{let e=[];return t.forEach(t=>{let n=t.val();e.push(n)}),e}}catch(e){return S("Kunde inte hämta användare, testa igen",5e3),[]}}async function sc(e){let t=r8(sa,sl);try{if(await sf(e.name)){S("Användarnamnet finns redan",5e3);return}await se(t,e)}catch(e){S("Kunde inte skapa användare, testa igen",5e3),console.log(e)}}async function su(e){let t=await sm(e.name);if(console.log(t),t){let n=r8(sa,`${sl}/${t}`);try{await sn(n,e)}catch(e){S("Kunde inte uppdatera användare, testa igen",5e3),console.log(e)}}else S("Användare hittades inte",5e3)}async function sd(e){let t=await sm(e),n=r8(sa,`${sl}/${t}`);try{await st(n)}catch(e){S("Kunde inte ta bort användare, testa igen",5e3),console.log(e)}}async function sp(e,t){let n=r8(sa,sl),i=await si(n);if(!i.exists())return!1;{let n=!1;return i.forEach(i=>{let r=i.val();r.name===e&&r.password===t&&(n=!0)}),n}}async function sf(e){let t=r8(sa,sl),n=await si(t);if(!n.exists())return!1;{let t=!1;return n.forEach(n=>{n.val().userName===e&&(t=!0)}),t}}async function sm(e){let t=r8(sa,sl),n=await si(t);if(!n.exists())return S("Kunde inte hitta användare, testa igen",5e3),null;let i=null;return n.forEach(t=>{if(t.val().name===e)return i=t.key,!0}),i}const s_={isInputEmpty:e=>!e.trim(),isUserNameValid:e=>/^[A-Za-z0-9_]+$/.test(e),isPasswordValid:e=>e.length>=6,isPasswordMatch:(e,t)=>e===t,isUserNameTaken:async e=>!!(await sh()).find(t=>t.name===e)},sg=document.querySelector("#topic"),sv=document.querySelector("#topicHeader"),sy=document.querySelector("#posts");function sb(){sg.classList.remove("hide"),sg.classList.add("flex"),document.querySelector(".startWrapper");let e=document.querySelector("#posts"),t=document.querySelector(".subjects");e.innerHTML="",t.innerHTML="",sv.innerHTML="";let n=document.querySelector(".userContainer"),i=document.querySelector(".userProfileComments");n&&(n.innerHTML="",i&&(i.innerHTML=""))}async function sC(e){let t=document.querySelector("#userName"),n=document.querySelector("#password");if(!t||!n)return;let i=t.value,r=n.value;if(s_.isInputEmpty(i)||s_.isInputEmpty(r)){S("Alla fälten måste vara ifyllda",5e3);return}await sp(i,r)?(localStorage.setItem("login",i),e.navigate(`/user/${i}`),console.log("router",e)):S("Fel användarnamn eller lösenord",5e3)}function sw(e){localStorage.removeItem("login"),e.navigate("/login"),sb()}function sI(){return!!localStorage.getItem("login")}function sE(){return localStorage.getItem("login")}var sT={};sT=new URL("black-panther.5ebfe666.png",import.meta.url).toString();var sS={};sS=new URL("red-panda.2102a999.png",import.meta.url).toString();var sk={};sk=new URL("babirusa.78f33283.png",import.meta.url).toString();const sx=e=>{let t;let n=document.querySelector(".mainMenu"),i=document.querySelector("#topMenuUser");if(!n||!i)return;let r=sI(),s=sE();t=r&&s?`<a href=/user/${s} data-navigo>Se profilsida</a>`:'<a href="/login" data-navigo>Logga in</a>',i.innerHTML=`${t}`,e.updatePageLinks()};function sP(e){switch(e){case"black-panther":default:return m(sT);case"red-panda":return m(sS);case"babirusa":return m(sk)}}const sN=e=>(e.image=sP(e.image),`
  ...
  `),sR=async(e,t)=>{let n=document.querySelector(".mainAside");if(!n){S("Kunde ej hitta mainAside",5e3);return}n.innerHTML="";try{if(!sI()){S("Du måste vara inloggad för att se dina uppgifter",5e3),e.navigate("/login");return}let i=(await sh()).find(e=>e.name===t);if(!i){S("Användaren finns inte",5e3);return}n.innerHTML=sN(i),sL(e),e.updatePageLinks()}catch(e){S("Kunde inte hämta användaren",5e3)}},sL=e=>{let t=document.querySelector("#logout");t&&t.addEventListener("click",()=>{sw(e),sD()})},sD=()=>{let e=document.querySelector(".mainAside");e&&(e.innerHTML="")},sA=async e=>{if(!document.querySelector(".mainAside"))return;let t=sE();null!==t&&(await sR(e,t),await sO(e),e.updatePageLinks())},sO=async e=>{if(!sI())return;let t=document.querySelector(".allUsers");if(!t){S("Det blev fel, försök igen",5e3);return}try{let n=await sh();t.innerHTML="<h3>Alla användare</h3><ul class='userList'></ul>";let i=t.querySelector(".userList");if(!i)return;n.forEach(e=>{let t=document.createElement("li"),n=document.createElement("a");n.href=`/user/${e.name}`,n.setAttribute("data-navigo",""),n.innerText=e.name,i.appendChild(t),t.appendChild(n)}),e.updatePageLinks(),function(e){let t=document.querySelector(".userList");t&&t.addEventListener("click",t=>{t.preventDefault();let n=t.target.innerText;e.navigate(`/user/${n}`)})}(e)}catch(e){console.error(e),S("Kunde inte hämta användare",5e3)}};async function sM(e){var t;let n=document.querySelector("#regUserName").value,i=document.querySelector("#regPassword").value,r=document.querySelector("#confirmPassword").value,s=document.querySelector('input[name="profileImage"]:checked');await sq(n,i,r)&&sc((t=s.value,{name:n,password:i,id:x(),image:t||"babirusa"})).then(()=>localStorage.setItem("login",n)).then(()=>e.navigate(`/user/${n}`)).then(()=>S("Användare skapad",5e3)).then(()=>sx(e)).then(()=>sA(e)).catch(e=>{console.error("Registration error:",e),S("Ett fel uppstod under registreringen",5e3)})}async function sq(e,t,n){return s_.isUserNameValid(e)?s_.isInputEmpty(e)||s_.isInputEmpty(t)||s_.isInputEmpty(n)?(S("Alla fälten måste vara ifyllda",5e3),!1):s_.isPasswordValid(t)?s_.isPasswordMatch(t,n)?!await s_.isUserNameTaken(e)||(S("Användarnamnet är redan taget",5e3),!1):(S("Lösenorden matchar inte",5e3),!1):(S("Lösenordet måste vara minst 6 tecken långt",5e3),!1):(S("Användarnamnet får bara innehålla bokstäver,siffror och understräck",5e3),!1)}async function sF(e,t){let n=document.querySelector(".mainContent");if(n){n.innerHTML="";try{let i=(await sh()).find(e=>e.name===t),r=sE();if(i?.name!==r){S("Du kan bara redigera din egen profil",5e3),e.navigate(`/user/${r}`);return}if(!i){S("Användaren finns inte",5e3);return}n.innerHTML=`
        <div class="editUser">
        ...
        `,sH(e,i),e.updatePageLinks()}catch(e){S("Kunde inte hämta användaren",5e3)}}}const sH=(e,t)=>{let n=document.querySelector("#editUpdate"),i=document.querySelector("#editDelete");n&&i&&(n.addEventListener("click",async n=>{n.preventDefault();let i=document.querySelector('input[name="profileImage"]:checked'),r=document.querySelector("#editPassword").value;if(r!==document.querySelector("#confirmEditPassword").value){S("Lösenorden matchar inte",5e3);return}let s={id:t.id,name:t.name,image:i?i.value:"black-panther",password:r||t.password};await su(s),e.navigate(`/user/${s.name}`),S("Profilen uppdaterad",5e3),T(!1,"#editUserContainer")}),i.addEventListener("click",async n=>{n.preventDefault(),await sd(t.name),sw(e),e.navigate("/login"),S("Användaren borttagen",5e3),T(!1,"#editUserContainer")}),k(".editImageRadio",".editImageLabel"))},sU="threadsv2";async function s$(){let e=r8(sa,sU);try{let t=await si(e);if(!t.exists())return S("Inga trådar hittades",5e3),[];{let e=[];return t.forEach(t=>{let n=t.val();e.push(n)}),e}}catch(e){return S("Kunde inte hämta trådar, testa igen",5e3),[]}}async function sB(e){let t=r8(sa,sU);try{await sn(se(t),e)}catch(e){S("Kunde inte skapa tråd, testa igen",5e3),console.log(e)}}async function sW(e){let t=r8(sa,sU),n=await si(t);if(!n.exists())return null;let i=null;return n.forEach(t=>{let n=t.val();if(n.id===e)return i=n,!0}),i}const sj="commentsv2";async function sz(){let e=r8(sa,sj);try{let t=await si(e);if(!t.exists())return[];{let e=[];return t.forEach(t=>{let n=t.val();e.push(n)}),e}}catch(e){return S("Kunde inte hämta kommentarer, testa igen",5e3),[]}}async function sV(e){let t=r8(sa,sj);try{await se(t,e)}catch(e){S("Kunde inte skapa kommentar, testa igen",5e3),console.log(e)}}async function sK(e){let t=await sY(e);if(!t){S("Kunde inte hitta kommentar, testa igen",5e3);return}let n=r8(sa,`${sj}/${t}`);try{await st(n)}catch(e){S("Kunde inte ta bort kommentar, testa igen",5e3),console.log(e)}}async function sY(e){let t=r8(sa,sj),n=await si(t);if(!n.exists())return null;let i=null;return n.forEach(t=>{t.val().id===e&&(i=t.key)}),i}async function sG(){let e=r8(sa,"forumv2");try{let t=await si(e);if(!t.exists())return S("Inga forum hittades",5e3),[];{let e=[];return t.forEach(t=>{let n=t.val();e.push(n)}),e}}catch(e){return S("Kunde inte hämta data",5e3),[]}}async function sQ(e,t){let n=document.querySelector(".userProfileComments");if(n){n.innerHTML="";try{if(!sI()){S("Du måste vara inloggad för att se profiler",5e3);return}for(let i of(await sz()).filter(t=>t.userId===e.id).sort((e,t)=>t.timeStamp-e.timeStamp).slice(0,3)){let r=await sW(i.threadId),s=(await sG()).find(e=>e.id===r?.forumId);if(!r||!s)return;n.innerHTML+=function(e,t,n,i){let r=E(e.timeStamp),s=t.name===sE();return`
    ...
    `;let s=i.querySelector(".deleteUserComment"),o=i.querySelector(".cancelDeleteComment");s&&o&&(s.addEventListener("click",async()=>{try{await sK(e),i.remove()}catch(e){S("Kunde inte radera inlägget",5e3),console.log(e)}}),o.addEventListener("click",()=>{console.log("cancel"),r.innerHTML=`
    ...
    `,sJ(t,n)}))}(parseInt(r),e,t)})}),i.forEach(e=>{e.addEventListener("click",async t=>{let n=e.getAttribute("data-comment-id");n&&(parseInt(n),function(){let e=document.querySelector(".editUserComment");e&&e.addEventListener("click",async t=>{e.getAttribute("data-comment-id")&&S("Funktionen är inte implementerad",5e3)})}())})})}async function sX(e,t){let n=document.querySelector(".userProfileThreads");if(n){n.innerHTML="";try{let i=(await s$()).filter(t=>t.userId===e.id).sort((e,t)=>t.timeStamp-e.timeStamp).slice(0,3);if(0===i.length)return;for(let e of(n.innerHTML="<h2>Trådar</h2>",i))n.innerHTML+=`
    <div class="thread">
            <a href="/threads/${e.id}" data-navigo>${e.title}</a>
    </div>
    `;t.updatePageLinks()}catch(e){console.error(e),S("Något gick fel, försök igen senare",5e3)}}}async function sZ(e,t){let n=document.querySelector(".mainContent");if(n){n.innerHTML="";try{sI()||S("Du måste vara inloggad för att se profiler",5e3);let i=(await sh()).find(t=>t.name===e);if(!i){S("Användaren finns inte",5e3);return}n.innerHTML=(i.image=sP(i.image),`
    <h1> Profil </h1>
...
`),sX(i,t),sQ(i,t),t.updatePageLinks()}catch(e){S("Något gick fel, försök igen",5e3)}}}const s0=()=>{let e=document.querySelector(".mainFooter");if(!e)return;let t=`
  <ul class="footerMenu">
    ...
    &copy; Omnitalk
  `;e.innerHTML=t},s1=document.querySelector("#posts");function s2(e,t,n){let i=document.createElement("h4");if(i.innerText=n,s1.append(i),document.querySelector("#formContainer"))for(let i of(function(e){console.log(e);let t=document.querySelector("#formContainer"),n=document.createElement("h3"),i=document.createElement("form");i.setAttribute("id","commentsForm"),n.innerHTML=`Skriv en kommentar i <br/> "${e}"`,i.innerHTML=`
    <div>
  ...
  `,t.append(n,i),i.addEventListener("submit",e=>{e.preventDefault(),i.querySelector("#postTextInput"),console.log(sE()),i.reset()})}(n),e))sh().then(e=>e.find(e=>e.id===i.userId)).then(e=>{!function(e,t,n,i){s1.classList.remove("hide");let r=s3("article","post"),s=s3("div","postHeader"),o=s3("div","postSubject"),a=document.createElement("span");a.setAttribute("data-navigo","");let l=s3("div","postDate"),h=s3("div","postBody"),c=s3("div","postUserInfo"),u=s3("a","postUserName");u.setAttribute("href",`/user/${i.name}`),u.setAttribute("data-navigo","");let d=document.createElement("div"),p=s3("img","postUserImg"),f=s3("div","postText"),m=document.createElement("p"),_=s3("div","postFooter"),g=document.createElement("div"),v=s3("a","postTopic");v.setAttribute("href",`/topic/${t}`),v.setAttribute("data-navigo","");let y=document.createElement("div"),b=s3("a","postFooterLink"),C=s3("a","postFooterLink");b.setAttribute("href","#"),b.setAttribute("data-navigo",""),C.setAttribute("href","#"),C.setAttribute("data-navigo",""),a.innerText=n;let w=E(e.timeStamp);l.innerText=`${w.time} | ${w.date}`,u.innerText=i.name,p.src=sP(i.image),m.innerText=e.comment,v.innerText=t;let I=i.name===sE();b.innerHTML=`<button class="editUserComment" data-comment-id="${e.id}" ${I?"":'style="display:none;"'}>Redigera</button>`,C.innerHTML=`<button class="deleteUserComment" data-comment-id="${e.id}" ${I?"":'style="display:none;"'}>Radera</button>`,s1.append(r),r.append(s,h,_),s.append(l,o),o.append(a),h.append(c,f),c.append(u,d),d.append(p),f.append(m),_.append(g,y),g.append(v),y.append(b,C)}(i,t,n,e)})}function s3(e,t){let n=document.createElement(e);return void 0!==t&&n.classList.add(t),n}function s5(e){return 1===e.length?e:e.sort((e,t)=>t.timeStamp-e.timeStamp)}function s4(e,t,n){if(sI())for(let i of e.slice(0,5))sz().then(s5).then(e=>e.find(e=>e.threadId===i.id)).then(e=>{(function(e,t,n){d=e.title,n.id;let i=document.createElement("div"),r=document.createElement("h4"),s=document.createElement("a");s.setAttribute("id",e.id.toString()),s.setAttribute("href",`/thread/${e.id}`),s.setAttribute("data-navigo",""),s.setAttribute("title",n.title);let o=document.createElement("p");document.createElement("p"),s.innerText=e.title,void 0!==t&&(o.innerText=t.comment.slice(0,50).trim()+"..."),s6.append(i),i.append(r,o),r.append(s)})(i,e,t)}),n.updatePageLinks();else{S("Du måste vara inloggad för att se forumet!");return}}const s6=document.querySelector(".subjects");async function s9(e,t){let n=parseInt(e),i=document.querySelector(".mainContent");if(i){i.innerHTML="";try{let e=(await s$()).filter(e=>e.forumId===n),i=(await sG()).find(e=>e.id===n);if(!i)return;s4(e,i,t)}catch(e){S("Kunde inte hämta trådar",5e3)}}}async function s8(e,t){let n=parseInt(e),i=document.querySelector(".mainContent");if(i){i.innerHTML="";try{let e=(await sz()).filter(e=>e.threadId===n),t=(await s$()).find(t=>t.id===e[0].threadId);if(!t)return;let i=t.title,r=await sG(),s=t.forumId,o=r.find(e=>e.id===s)?.title;if(!o)return;s2(e,o,i)}catch(e){S("Kunde inte hämta kommentarer",5e3)}}}s6.addEventListener("click",e=>{let t=document.querySelector("#formContainer");e.preventDefault(),sy.innerHTML="",t.innerHTML="";let n=parseInt(e.target.id),i=e.target.title;sz().then(e=>e.filter(e=>e.threadId===n)).then(s5).then(e=>s2(e,i,d))});const s7=document.querySelector("#topicHeader");async function oe(e){let t=await sG();document.querySelector(".topicsMenuWrapper").innerHTML="",function(e,t){for(let n of e)(function(e,t){let n=document.querySelector(".topicsMenuWrapper"),i=document.createElement("div");i.classList.add("topicMenubox");let r=document.createElement("a");r.setAttribute("id",e.id.toString()),r.setAttribute("href",`/topic/${e.id}`),r.setAttribute("data-navigo",""),r.innerText=e.title,n.append(i),i.append(r),r.addEventListener("click",n=>{if(n.preventDefault(),sI()){n.target.id;let i=document.createElement("h2"),r=document.createElement("p");r.classList.add("topicDesc"),i.innerText=e.title,r.innerText=e.description,sb(),s7.append(i,r),s$().then(t=>t.filter(t=>t.forumId===e.id)).then(n=>s4(n,e,t))}else{S("Du måste vara inloggad för att se inlägg!");return}t.updatePageLinks()})})(n,t)}(t,e)}async function ot(e,t){let n=document.querySelector("#postSendButton");n&&n.addEventListener("click",async n=>{n.preventDefault();let i=await on(e);if(!i)return;let r=await oi(i);r&&(sB(i),sV(r),t.navigate(`/thread/${i.id}`))})}async function on(e){let t=sE();if(!t){S("Du måste vara inloggad för att skapa en tråd");return}try{let n=await sh(),i=n.find(e=>e.name===t)?.id;if(!i||!document.querySelector("#createThreadForm"))return;let r=document.querySelector("#subjectHeaderInput").value,s=parseInt(e);if(!r){S("Du måste ange ett ämne");return}return function(e,t,n){let i=Date.now();return{title:e,id:x(),userId:t,forumId:n,timeStamp:i}}(r,i,s)}catch(e){S("Något gick fel, testa igen",5e3)}}async function oi(e){let t=e.userId,n=e.id,i=document.querySelector("#postText").value;if(!i){S("Du måste skriva något i inlägget");return}return function(e,t,n){let i=Date.now();return{id:x(),comment:e,timeStamp:i,threadId:t,userId:n}}(i,n,t)}async function or(e){sx(e),await sA(e),s0(),await oe(e)}async function os(e){sI()?(await or(e),function(){let e=document.querySelector("#mainContent");e&&(e.innerHTML="",e.innerHTML=`
...
`)}()):e.navigate("/login")}async function oo(e){sI()?e.navigate("/"):(!function(e){let t=document.querySelector(".mainContent");t&&(t.innerHTML="",t.innerHTML=`
    <div class="login">
        <h1>Login</h1>
        <form>
    ...
    `,function(e){let t=document.querySelector("#login"),n=document.querySelector("#register");t&&!t.getAttribute("data-listener")&&(t.setAttribute("data-listener","true"),t.addEventListener("click",t=>{t.preventDefault(),sC(e)})),n&&!n.getAttribute("data-listener")&&(n.setAttribute("data-listener","true"),n.addEventListener("click",t=>{t.preventDefault(),e.navigate("/register")}))}(e),e.updatePageLinks())}(e),await or(e))}async function oa(e){sI()?e.navigate("/"):(!function(e){let t=document.querySelector(".mainContent");t&&(t.innerHTML="",t.innerHTML=`
  <div class="register">
  ...
  <form>
    ...
    `,function(e){let t=document.querySelector("#registerBtn"),n=document.querySelector("#backBtn");function i(t){t.preventDefault(),sM(e)}function r(t){t.preventDefault(),e.navigate("/login")}t?(t.removeEventListener("click",i),t.addEventListener("click",i)):console.error("Register button not found"),n?(n.removeEventListener("click",r),n.addEventListener("click",r)):console.error("Back button not found"),k(".regImageRadio",".regImageLabel")}(e),e.updatePageLinks())}(e),await or(e))}async function ol(e,t){sI()?(await sZ(t.data.id,e),await or(e),function(){let e=document.querySelector("#topic"),t=document.querySelector("#posts");e&&t&&(e.classList.remove("flex"),e.classList.add("hide"),t.classList.remove("flex"),t.classList.add("hide"))}()):e.navigate("/login")}async function oh(e,t){sI()?(await sF(e,t.data.id),await or(e)):e.navigate("/login")}async function oc(e,t){sI()?(await s9(t.data.id,e),await function(e,t){let n=document.querySelector("#formContainer");n&&(n.innerHTML=`
  ...
  `,ot(e,t),t.updatePageLinks())}(t.data.id,e),await or(e),function(e){let t=document.querySelector("#topic");t&&(e?(t.classList.remove("hide"),t.classList.add("flex")):(t.classList.remove("flex"),t.classList.add("hide")))}(!0)):e.navigate("/login")}async function ou(e,t){sI()?(await s8(t.data.id,e),await or(e),function(e){let t=document.querySelector("#posts");t&&(e?t.classList.remove("hide"):t.classList.add("hide"))}(!0)):e.navigate("/login")}"undefined"!=typeof self&&self,function(e){e.on({"/":()=>{os(e)},"/login":()=>{oo(e)},"/register":()=>{oa(e)},"/user/:id":t=>{ol(e,t)},"/user/:id/edit":t=>{oh(e,t)},"/faq":()=>{!function(){let e=document.querySelector(".mainContent");e&&(e.innerHTML="",e.innerHTML=`
