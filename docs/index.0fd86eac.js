 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class nw{getCompare(){return this.compare.bind(this)}indexedValueChanged(e,t){let n=new nb(tR,e),i=new nb(tR,t);return 0!==this.compare(n,i)}minPost(){return nb.MIN}}class nI extends nw{static get __EMPTY_NODE(){return n}static set __EMPTY_NODE(e){n=e}compare(e,t){return tD(e.name,t.name)}isDefinedOn(e){throw W("KeyIndex.isDefinedOn not expected to be called.")}indexedValueChanged(e,t){return!1}minPost(){return nb.MIN}maxPost(){return new nb(tL,n)}makePost(e,t){return B("string"==typeof e,"KeyIndex indexValue must always be a string."),new nb(e,n)}toString(){return".key"}}const nE=new nI;/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class extends nw{compare(e,t){let n=e.node.compareTo(t.node);return 0===n?tD(e.name,t.name):n}isDefinedOn(e){return!0}indexedValueChanged(e,t){return!e.equals(t)}minPost(){return nb.MIN}maxPost(){return nb.MAX}makePost(e,t){return new nb(t,n$(e))}toString(){return".value"}};/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
  ...
  `),sk=async(e,t)=>{let n=document.querySelector(".mainAside");if(!n){T("Kunde ej hitta mainAside",5e3);return}n.innerHTML="";try{if(!sv()){T("Du måste vara inloggad för att se dina uppgifter",5e3),e.navigate("/login");return}let i=(await sl()).find(e=>e.name===t);if(!i){T("Användaren finns inte",5e3);return}n.innerHTML=sT(i),sS(e),e.updatePageLinks()}catch(e){T("Kunde inte hämta användaren",5e3)}},sS=e=>{let t=document.querySelector("#logout");t&&t.addEventListener("click",()=>{sg(e),sP()})},sP=()=>{let e=document.querySelector(".mainAside");e&&(e.innerHTML="")},sx=async e=>{let t=document.querySelector(".mainAside");if(!t)return;let n=sy();if(null===n){t.innerHTML="";return}await sk(e,n),await sN(e),e.updatePageLinks()},sN=async e=>{if(!sv())return;let t=document.querySelector(".allUsers");if(!t){T("Det blev fel, försök igen",5e3);return}try{let n=await sl();t.innerHTML="<h3>Alla användare</h3><ul class='userList'></ul>";let i=t.querySelector(".userList");if(!i)return;n.forEach(e=>{let t=document.createElement("li"),n=document.createElement("a");n.href=`/user/${e.name}`,n.setAttribute("data-navigo",""),n.innerText=e.name,i.appendChild(t),t.appendChild(n)}),function(e){let t=document.querySelector(".userList");t&&t.addEventListener("click",t=>{t.preventDefault();let n=t.target.innerText;e.navigate(`/user/${n}`)})}(e),e.updatePageLinks()}catch(e){T("Kunde inte hämta användare",5e3)}};async function sR(e){var t;let n=document.querySelector("#regUserName").value,i=document.querySelector("#regPassword").value,r=document.querySelector("#confirmPassword").value,s=document.querySelector('input[name="profileImage"]:checked');await sL(n,i,r)&&sh((t=s.value,{name:n,password:i,id:S(),image:t||"babirusa"})).then(()=>localStorage.setItem("login",n)).then(()=>e.navigate(`/user/${n}`)).then(()=>T("Användare skapad",5e3)).then(()=>sI(e)).then(()=>sx(e)).catch(e=>{console.error("Registration error:",e),T("Ett fel uppstod under registreringen",5e3)})}async function sL(e,t,n){return s_.isUserNameValid(e)?s_.isInputEmpty(e)||s_.isInputEmpty(t)||s_.isInputEmpty(n)?(T("Alla fälten måste vara ifyllda",5e3),!1):s_.isPasswordValid(t)?s_.isPasswordMatch(t,n)?!await s_.isUserNameTaken(e)||(T("Användarnamnet är redan taget",5e3),!1):(T("Lösenorden matchar inte",5e3),!1):(T("Lösenordet måste vara minst 6 tecken långt",5e3),!1):(T("Användarnamnet får bara innehålla bokstäver,siffror och understräck",5e3),!1)}async function sD(e,t){let n=document.querySelector(".mainContent");if(n){n.innerHTML="";try{let i=(await sl()).find(e=>e.name===t),r=sy();if(i?.name!==r){T("Du kan bara redigera din egen profil",5e3),e.navigate(`/user/${r}`);return}if(!i){T("Användaren finns inte",5e3);return}n.innerHTML=`
        <div class="editUser">
            ...
            <label for="image2" class="editImageLabel">
              ...
              <img src="${_(sb)}" alt="Profile Image 2">
            </label>
...
`),sK(i,t),sz(i,t),t.updatePageLinks()}catch(e){T("Något gick fel, försök igen",5e3)}}}const sG=()=>{let e=document.querySelector(".mainFooter");if(!e)return;let t=`
  ...
  `,s7(e,t),t.updatePageLinks())}async function s8(e){try{let t=(await sj()).find(t=>t.id===parseInt(e));if(!t)return"forum";return t.title}catch(e){return T("Något gick fel, testa igen",5e3),"forum"}}async function s7(e,t){let n=document.querySelector("#postSendButton");n&&n.addEventListener("click",async n=>{n.preventDefault();let i=await oe(e);if(!i)return;let r=await ot(i);r&&(sq(i),s$(r),t.navigate(`/thread/${i.id}`),T("Inlägget skapat",5e3),function(){let e=document.querySelector("#threadContainer");if(console.log("Form:",e),!e){console.error("Form container not found");return}e.reset(),console.log("Form reset")}())})}async function oe(e){let t=sy();if(!t){T("Du måste vara inloggad för att skapa en tråd");return}try{let n=await sl(),i=n.find(e=>e.name===t)?.id;if(!i||!document.querySelector("#threadContainer"))return;let r=document.querySelector("#subjectHeaderInput").value,s=parseInt(e);if(!r){T("Du måste ange ett ämne");return}return function(e,t,n){let i=Date.now();return{title:e,id:S(),userId:t,forumId:n,timeStamp:i}}(r,i,s)}catch(e){T("Något gick fel, testa igen",5e3)}}async function ot(e){let t=e.userId,n=e.id,i=document.querySelector("#postText").value;if(!i){T("Du måste skriva något i inlägget");return}return function(e,t,n){let i=Date.now();return{id:S(),comment:e,timeStamp:i,threadId:t,userId:n}}(i,n,t)}async function on(e){sI(e),await sx(e),sG(),await s6(e)}function oi(e){let t=document.querySelector("#topic");t&&(e?(t.classList.remove("hide"),t.classList.add("flex")):(t.classList.remove("flex"),t.classList.add("hide")))}function or(e){let t=document.querySelector("#posts");t&&(e?t.classList.remove("hide"):t.classList.add("hide"))}async function os(e){sv()?(await on(e),or(!1),function(){let e=document.querySelector("#mainContent");e&&(e.innerHTML="",e.innerHTML=`
...
`)}()):e.navigate("/login")}async function oo(e){sv()?e.navigate("/"):(!function(e){let t=document.querySelector(".mainContent");t&&(t.innerHTML="",t.innerHTML=`
    <div class="login">
        <h1>Login</h1>
        <form>
  ...
  <form>
    ...
    </div>`)})(),oi(!1),or(!1)},"/kontakt":()=>{(function(){let e=document.querySelector(".mainContent");e&&(e.innerHTML="",e.innerHTML=`
      <form id="contactForm">
